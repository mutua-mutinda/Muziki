  ...
  <header id="header" class="bg-gray-700">
    <nav class="container mx-auto flex items-center py-5 px-4">
      ...
      <div class="flex flex-grow justify-between items-center">
        ...
        <!-- dummy el to enable signin button align right -->
        <div v-show="!userLoggedIn" class="flex flex-grow">
          <span class="sr-ony"></span>
        </div>

        <div v-show="userLoggedIn" class="flex flex-row">
          <router-link class="px-2 text-white dark:text-gray-300 dark:hover:text-amber-500" to="/manage">Manage</router-link>
        </div>

        <!-- align right -->
        <div  class="flex flex-row justify-end mt-1">
          <span v-show="!userLoggedIn">
            <a  class="px-2 text-white dark:text-gray-300 dark:hover:text-amber-500" href="/#/signin" @click.prevent="toggleAuthModal" >Sign In</a >
          </span>
          <span v-show="userLoggedIn" >
            <a class="px-2 text-white dark:text-gray-300 dark:hover:text-amber-500" href="/#/logout" @click.prevent="signout">Logout</a>
          </span>
        </div>
          ...
          <button type="button" id="headlessui-listbox-button-9" aria-haspopup="true" aria-expanded="false" aria-labelledby="headlessui-listbox-label-8 headlessui-listbox-button-9">
            ...
            <!-- dark -->
              <span class="hidden dark:inline">
                <svg viewBox="0 0 24 24" fill="none" class="w-6 h-6"><path fill-rule="evenodd" clip-rule="evenodd" d="M17.715 15.15A6.5 6.5 0 0 1 9 6.035C6.106 6.922 4 9.645 4 12.867c0 3.94 3.153 7.136 7.042 7.136 3.101 0 5.734-2.032 6.673-4.853Z" class="fill-sky-400/20"></path>
